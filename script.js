const calculatorConfig = {
  ...
  revenuePerService: null,
  marginPerService: null,
  recommendedPackCount: 5,
  pricing: {
    regularUnitPrice: 11.21,
    packageUnitPrice: 8.12,
    unitsPerPack: 10,
    completePackagePackCount: 5,
    completePackagePrice: 406.0,
    completePackageRegularValue: 560.5
  },
  ctaButtonLabel: "Zusammenstellung anfragen",
  requestButtonLabel: "Berechnung senden",
  introText: {
    missingPotentialValues:
      "Umsatz je Service und Margendefinition sind noch nicht fachlich hinterlegt. Sobald diese Werte ergänzt sind, rechnet der Rechner das Potenzial automatisch.",
    priceBasis:
      "Preisvorgabe aktuell: regulär 11,21 € pro Stück, Paketpreis 8,12 € pro Stück bei Abnahme eines 10er-Packs. Ein 10er-Pack kostet 81,20 €. Das vollständige Paket aus fünf 10er-Packs kostet 406,00 € statt regulär 560,50 €."
  },
  products: [
    {
      articleNumber: "1201",
    ...
    {
      articleNumber: "1199",
      name: "Produktname ergänzen",
      description: "Artikel 1199 wurde im aktuellen Artikelstamm nicht gefunden. Produktdaten fachlich ergänzen oder Artikelnummer prüfen.",
      shortBenefit: "Noch nicht im Artikelstamm belegt.",
      articleMasterNameGerman: null,
      packageContent: "Inhalt ergänzen",
      articleMasterSalesUnit: null,
...
function getProductRows() {
  return calculatorConfig.products.map((product) => {
    ...
    const packageUnitPrice = product.packageUnitPrice ?? calculatorConfig.pricing.packageUnitPrice;
...
function getCoveredServices(productRows) {
  const rowsWithRequirement = productRows.filter((product) => Number(product.unitsRequiredPerService) > 0);

  if (rowsWithRequirement.length !== productRows.length) {
    return null;
  }

  return Math.floor(
    rowsWithRequirement.reduce((minimumServices, product) => {
      const coveredByProduct = product.units / product.unitsRequiredPerService;
      return Math.min(minimumServices, coveredByProduct);
...
function getCalculation() {
  ...
  const totalPacks = products.reduce((sum, product) => sum + product.packs, 0);
  ...
  const coveredServices = getCoveredServices(products);
  const reachMonths =
  ...
  const annualRevenueCents =
    calculatorConfig.revenuePerService === null
      ? null
      : serviceVolume.servicesPerYear * toCents(calculatorConfig.revenuePerService);
  const annualMarginCents =
    calculatorConfig.marginPerService === null
      ? null
      : serviceVolume.servicesPerYear * toCents(calculatorConfig.marginPerService);
  const packageRevenueCents =
    coveredServices === null || calculatorConfig.revenuePerService === null
      ? null
      : coveredServices * toCents(calculatorConfig.revenuePerService);
  ...
  const cards = [
    {
      title: "Paketumfang",
      rows: [
        ["Packs", formatNumber(calculation.totalPacks)],
    ...
    {
      title: "Preisvorteil",
      note: `Standardpaket: ${formatNumber(calculatorConfig.pricing.completePackagePackCount)} x 10er-Pack`,
      rows: [
    ...
    },
    {
      title: "Potenzial",
      rows: [
        ["Umsatz", formatMoneyFromCents(calculation.totals.packageRevenueCents)],
        ["Marge", formatMoneyFromCents(calculation.totals.packageMarginCents)]
      ]
    }
  ];
        ...
        <section class="product-card" data-article-number="${product.articleNumber}">
          ...
          <div class="product-body">
            ...
            <p class="product-detail">
              ${product.articleMasterNameGerman ? `${product.articleMasterNameGerman} · ` : ""}
              ${product.packageContent}
              ${product.articleMasterSalesUnit ? ` · VE laut Artikelstamm: ${formatNumber(product.articleMasterSalesUnit)}` : ""}
            </p>
            <p class="product-detail">${product.shortBenefit}</p>

          </div>

          <div class="product-footer">
            <div class="quantity-control">
              <button class="step-btn" type="button" data-product-step="${product.articleNumber}" data-step="-1" aria-label="Packs für Artikel ${product.articleNumber} verringern">−</button>
              <label class="sr-only" for="packs-${product.articleNumber}">Packs für Artikel ${product.articleNumber}</label>
              ...
              <button class="step-btn" type="button" data-product-step="${product.articleNumber}" data-step="1" aria-label="Packs für Artikel ${product.articleNumber} erhöhen">+</button>
            </div>

            <div class="product-meta">
              <div>
                <span>Packs</span>
                <strong>${formatNumber(product.packs)}</strong>
              </div>
              <div>
                <span>Stückzahl</span>
              ...
              <div>
                <span>10er-Pack</span>
                <strong>${formatMoneyFromCents(product.unitsPerPack * toCents(product.packageUnitPrice))}</strong>
              </div>
              <div>
                <span>Regulär/Stück</span>
                <strong>${formatMoneyFromCents(toCents(product.regularUnitPrice))}</strong>
              </div>
              <div>
                <span>Warenwert</span>
                <strong>${formatMoneyFromCents(product.regularValueCents)}</strong>
              </div>
              <div>
                <span>Aktionspreis</span>
                <strong>${formatMoneyFromCents(product.packageCostCents)}</strong>
              </div>
            </div>
          </div>
        </section>
...
function renderRecommendedStatus(totalPacks) {
  ...
  if (difference > 0) {
    elements.recommendedStatus.textContent = `Noch ${formatNumber(difference)} Packs bis zur empfohlenen Paketgröße`;
  } else if (difference < 0) {
    elements.recommendedStatus.textContent = `${formatNumber(Math.abs(difference))} Packs über der empfohlenen Paketgröße`;
  } else {
    elements.recommendedStatus.textContent = "Empfohlene Paketgröße erreicht";
  }
}
...
function renderPotential(calculation) {
  ...
  elements.potentialHint.textContent =
    calculation.totals.annualRevenueCents === null || calculation.totals.annualMarginCents === null
      ? `${calculatorConfig.introText.missingPotentialValues} ${calculatorConfig.introText.priceBasis}`
      : calculatorConfig.introText.priceBasis;
}
...
function render() {
  ...
  renderOfferMetrics(calculation);
...
function handleSubmit(event) {
  ...
  sendRequest(payload);
  elements.formMessage.textContent = "Die Berechnung wurde vorbereitet. Die Daten wurden in der Browser-Konsole ausgegeben.";
  elements.formMessage.className = "form-message success";
  form.reset();
}

function sendRequest(payload) {
  console.log("Additive-Rechner Anfrage", payload);
}
