...
function renderProductTotals(calculation) {
  ...
  }).format(savingsPercent);

  elements.productTotals.innerHTML = `
    <div class="sum-item sum-highlight">
      <span>Stück gesamt</span>
      <strong>${formatNumber(calculation.totalUnits)}</strong>
    </div>
    <div class="sum-item">
      <span>Stattpreis</span>
      <strong>${formatMoneyFromCents(calculation.totals.regularValueCents)}</strong>
    </div>
    <div class="sum-item">
    ...
    <div class="sum-item">
      <span>Ersparnis €</span>
      <strong>${formatMoneyFromCents(calculation.totals.savingsCents)}</strong>
    </div>
    <div class="sum-item">
      <span>Ersparnis %</span>
      <strong>${savingsPercentText} %</strong>
    </div>
    <div class="sum-item sum-highlight">
      <span>Umsatzpotential</span>
      <strong>${formatMoneyFromCents(calculation.totals.packageRevenueCents)}</strong>
    </div>
    <div class="sum-item sum-highlight">
      <span>Dein Profit</span>
      <strong>${formatMoneyFromCents(calculation.totals.packageMarginCents)}</strong>
    </div>
  `;
}
...
function renderRecommendedStatus(totalPacks) {
  ...
  const extraCartonText = extraCartons === 1 ? "1 zusätzlicher Karton" : `${formatNumber(extraCartons)} zusätzliche Kartons`;

  if (difference > 0) {
    elements.recommendedStatus.className = "status-pill status-missing";
    elements.recommendedStatus.textContent = `Nur noch ${missingCartonText}, um dein vollständiges Paket abzuschließen.`;
  } else if (difference < 0) {
    elements.recommendedStatus.className = "status-pill status-extra";
    elements.recommendedStatus.textContent = `Du hast ${extraCartonText} ausgewählt. Diese werden zu denselben Konditionen berechnet.`;
  } else {
    elements.recommendedStatus.className = "status-pill status-complete";
    elements.recommendedStatus.textContent = "Perfekt – dein vollständiges 10-Karton-Paket ist komplett.";
  }
}
...
function renderCta(calculation) {
  ...
  elements.ctaText.innerHTML = `
    <p class="cta-summary">Mit deiner gewählten Zusammenstellung deckst du voraussichtlich <strong>${coveredServicesText}</strong> ab. Fordere jetzt die Zusammenstellung an oder exportiere die Berechnung als PDF.</p>
  `;
}
...
function buildRequestEmailBody(payload) {
  ...
  return [
    ...
    `Umsatzpotential mit Paket: ${formatMoneyValue(payload.totals.packageRevenue)}`,
    `Dein Profit mit Paket: ${formatMoneyValue(payload.totals.packageMargin)}`,
    `Jahresumsatz laut Service-Eingabe: ${formatMoneyValue(payload.totals.annualRevenue)}`,
    `Jahresmarge laut Service-Eingabe: ${formatMoneyValue(payload.totals.annualMargin)}`,
