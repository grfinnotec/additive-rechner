...
const elements = {
  ...
  productReach: document.querySelector("#productReach"),
...
function renderProductReach(calculation) {
  const reachMonths =
    calculation.reachMonths === null
      ...
      : new Intl.NumberFormat(calculatorConfig.locale, {
        ...
        }).format(calculation.reachMonths);

  elements.productReach.innerHTML = `Mit deiner unten ausgewählten Auswahl hast du <strong>${formatNumber(
    calculation.coveredServices
  )} Services</strong> abgedeckt für einen Zeitraum von <strong>${reachMonths} Monaten</strong>.`;
}
...
function renderProductTotals(calculation) {
  elements.productTotals.innerHTML = `
    ...
    <div class="sum-spacer" aria-hidden="true"></div>
    <div class="sum-item">
      <span>Kartons gesamt</span>
      <strong>${formatNumber(calculation.totalPacks)}</strong>
    </div>
    <div class="sum-item">
...
function renderRecommendedStatus(totalPacks) {
  ...
  if (difference > 0) {
    ...
    elements.recommendedStatus.textContent = "Perfekt – Ihr vollständiges 10-Karton-Paket ist komplett.";
  }
}
...
function renderCta(calculation) {
  ...
  const revenueText = formatMoneyFromCents(calculation.totals.packageRevenueCents);
  const marginText = formatMoneyFromCents(calculation.totals.packageMarginCents);

  elements.ctaText.innerHTML = `
    <p class="cta-summary">Mit der gewählten Zusammenstellung können Sie voraussichtlich <strong>${coveredServicesText}</strong> abdecken.</p>
    <div class="cta-metrics" aria-label="Potenzial der gewählten Zusammenstellung">
      <div class="cta-metric">
        <span>Umsatzpotenzial</span>
        <strong>${revenueText}</strong>
      </div>
      <div class="cta-metric">
        <span>Margenpotenzial</span>
...
function render() {
  ...
  renderProductReach(calculation);
...
function buildRequestEmailBody(payload) {
  ...
  return [
    ...
    `Margenpotential mit Paket: ${formatMoneyValue(payload.totals.packageMargin)}`,
