const calculatorConfig = {
  ...
  pricing: {
    ...
    unitsPerPack: 10,
    bulkCartonThreshold: 1,
    completePackagePackCount: 5,
    completePackagePrice: 406,
  ...
  introText: {
    priceBasis:
      "Basis: 250 Arbeitstage pro Jahr. Pro Service wird eine Flasche gerechnet. Eine Flasche kostet regulär 11,21 €. Im 10er-Pack wird mit 8,12 € je Flasche gerechnet."
  },
  products: [
    {
      articleNumber: "1201",
...
function renderRecommendedStatus(totalPacks) {
  ...
  if (difference > 0) {
    ...
    elements.recommendedStatus.textContent =
      difference === 1
        ? "Noch 1 Pack bis zum vollständigen Paket"
        : `Noch ${formatNumber(difference)} Packs bis zum vollständigen Paket`;
  } else if (difference < 0) {
    elements.recommendedStatus.className = "status-pill status-extra";
    const extraPacks = Math.abs(difference);
    elements.recommendedStatus.textContent =
      extraPacks === 1
        ? "+1 Pack über dem Standardpaket"
        : `+${formatNumber(extraPacks)} Packs über dem Standardpaket`;
  } else {
    elements.recommendedStatus.className = "status-pill status-complete";
    elements.recommendedStatus.textContent = "Vollständiges 5-Pack-Paket erreicht";
  }
}
