...
function renderRecommendedStatus(totalPacks) {
  ...
  const missingCartons = Math.max(0, difference);
  const extraCartons = Math.abs(Math.min(0, difference));
  const missingCartonText = missingCartons === 1 ? "1 Karton" : `${formatNumber(missingCartons)} Kartons`;
